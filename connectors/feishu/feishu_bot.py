...
def send_workflow_card(
    ...
    """
    Send an interactive card with a "Run in BioMate" button (deep link) for a
        ...
        "elements": [
            {
                ...
                "actions": [
                    {
                        "tag": "button",
                        "text": {"tag": "plain_text", "content": "在 BioMate 中运行 / Run in BioMate"},
                        "type": "primary",
                        "url": deep_link,
                    }
                ],
...
def handle_message_event(event: Dict[str, Any], _base_url_override: Optional[str] = None) -> None:
    ...
    reply_text, workflow_id = _open_claw_query(
